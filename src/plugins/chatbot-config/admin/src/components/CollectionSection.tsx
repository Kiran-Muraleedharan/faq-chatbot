import React from 'react';
import { Box, Typography, Flex, Checkbox, Divider, Accordion, Button } from '@strapi/design-system';
import { Plus, Pencil } from '@strapi/icons';

interface FieldConfig {
  name: string;
  enabled: boolean;
}

interface CollectionConfig {
  uid: string;
  name: string;
  fields: FieldConfig[];
}

interface CollectionSectionProps {
  collections: CollectionConfig[];
  onToggleField: (uid: string, fieldName: string) => void;
  onToggleAll: (uid: string, value: boolean) => void;
  onAddClick: () => void; // <--- This was missing in your interface
}

type CheckboxValue = boolean | 'indeterminate';

const CollectionSection: React.FC<CollectionSectionProps> = ({ 
  collections, 
  onToggleField, 
  onToggleAll,
  onAddClick
}) => {
  return (
    <Box background="neutral0" shadow="filterShadow" hasRadius paddingBottom={collections.length > 0 ? 4 : 0} marginBottom={6}>
      <Box padding={6}>
        <Flex justifyContent="space-between" alignItems="center">
          <Typography variant="delta" fontWeight="bold">
            Collections
          </Typography>
          <Button 
            variant="secondary" 
            // Show Pencil if collections exist, Plus if empty
            startIcon={collections.length > 0 ? <Pencil /> : <Plus />} 
            onClick={onAddClick}
          >
            {collections.length > 0 ? "Edit Collections" : "Add Collection"}
          </Button>
        </Flex>
      </Box>

      {collections.length > 0 && (
        <Accordion.Root type="multiple">
          {collections.map((c) => {
            const allChecked = c.fields.every((f) => f.enabled);
            const someChecked = c.fields.some((f) => f.enabled);

            return (
              <Accordion.Item key={c.uid} value={c.uid}>
                <Accordion.Header>
                  <Accordion.Trigger>
                    <Box paddingLeft={2} textAlign="left">
                      <Typography variant="delta" fontWeight="bold" display="block">
                        {c.name}
                      </Typography>
                      <Typography variant="pi" textColor="neutral600">
                        Define all allowed fields for {c.name}.
                      </Typography>
                    </Box>
                  </Accordion.Trigger>
                </Accordion.Header>

                <Accordion.Content>
                  <Box background="neutral100" padding={4}>
                    <Flex justifyContent="space-between" alignItems="center" paddingBottom={2}>
                      <Typography variant="sigma" textColor="neutral600">
                        {c.name.toUpperCase()} FIELDS
                      </Typography>
                      <Checkbox
                        checked={allChecked}
                        indeterminate={!allChecked && someChecked}
                        onCheckedChange={(value: CheckboxValue) => onToggleAll(c.uid, value === true)}
                      >
                        Select all
                      </Checkbox>
                    </Flex>
                    <Divider marginBottom={3} />
                    <Box paddingTop={2}>
                      <Flex gap={4} wrap="wrap">
                        {c.fields.map((f) => (
                          <Box key={f.name} style={{ minWidth: '140px' }}>
                            <Flex alignItems="center" gap={2}>
                              <Checkbox 
                                checked={f.enabled} 
                                onCheckedChange={() => onToggleField(c.uid, f.name)} 
                              />
                              <Typography variant="omega">{f.name}</Typography>
                            </Flex>
                          </Box>
                        ))}
                      </Flex>
                    </Box>
                  </Box>
                </Accordion.Content>
              </Accordion.Item>
            );
          })}
        </Accordion.Root>
      )}
    </Box>
  );
};

export default CollectionSection;